...
CASE_RE = re.compile(SOL_STR + r"(CASE|TYPE\s*IS|CLASS)\s*(\(.+\)|DEFAULT)" + EOL_STR, RE_FLAGS)
ENDSEL_RE = re.compile(SOL_STR + r"END\s*SELECT" + EOL_STR, RE_FLAGS)

ASSO_RE = re.compile(SOL_STR + r"(\w+\s*:)?\s*ASSOCIATE\s*(\(.*\))?" + EOL_STR, RE_FLAGS)
ENDASSO_RE = re.compile(SOL_STR + r"END\s*ASSOCIATE(\s+\w+)?" + EOL_STR, RE_FLAGS)
...
TYPE_RE = re.compile(
    SOL_STR +
    r"TYPE((\s*,\s*(ABSTRACT)\s*)|(\s*,\s*(BIND\s*\(\s*C\s*\)|EXTENDS\s*\(.*\)))|(\s*,\s*(PUBLIC|PRIVATE)\s*))*(\s*::\s*|\s+)\w+" + EOL_STR,
    RE_FLAGS)
...
# two-sided operators
LR_OPS_RE = [REL_OP_RE, LOG_OP_RE, PLUSMINUS_RE, MULTDIV_RE]

# markups to deactivate formatter
NO_ALIGN_RE = re.compile(SOL_STR + r"&\s*[^\s*]+")

# combine regex that define subunits
NEW_SCOPE_RE =      [IF_RE,    DO_RE,    SELCASE_RE, SUBR_RE,     FCT_RE,      MOD_RE,      PROG_RE,     INTERFACE_RE,    TYPE_RE,     ENUM_RE,    ASSO_RE,    BLK_RE]
CONTINUE_SCOPE_RE = [ELSE_RE,  None,     CASE_RE,    CONTAINS_RE, CONTAINS_RE, CONTAINS_RE, CONTAINS_RE, None,            CONTAINS_RE, None,       None,       None]
END_SCOPE_RE =      [ENDIF_RE, ENDDO_RE, ENDSEL_RE,  ENDSUBR_RE,  ENDFCT_RE,   ENDMOD_RE,   ENDPROG_RE,  ENDINTERFACE_RE, ENDTYPE_RE,  ENDENUM_RE, ENDASSO_RE, ENDBLK_RE]
...
class F90Aligner(object):
    ...
    def process_lines_of_fline(self, f_line, lines, rel_ind, line_nr):
        ...
        is_decl = VAR_DECL_RE.match(f_line) or PUBLIC_RE.match(f_line) or PRIVATE_RE.match(f_line)
    ...
    while 1:
        ...
        for ind, line, orig_line in zip(indent, lines, orig_lines):
            ...
            if ind_use + line_length <= 133:  # 132 plus 1 newline char
                ...
                logger.warning(("%s:%d: auto indentation failed"
...
def run(argv=None):
    ...
    If no files are given, stdin is used.
             ...
             Amount of whitespace controlled by --whitespace=0,1,2,3.
